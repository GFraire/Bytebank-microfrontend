...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ 
  ...
  return (
    <header className="bg-white shadow-sm border-b border-gray-200 px-3 py-2 md:px-6 md:py-4" role="banner">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2 md:space-x-4">
          <div className="w-8 h-8 md:w-10 md:h-10 bg-green-100 rounded-lg flex items-center justify-center" aria-hidden="true">
            {config.icon}
          </div>
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-900" id="page-title">{config.title}</h1>
            <p className="text-xs md:text-sm text-gray-600" aria-describedby="page-title">{config.subtitle}</p>
          </div>
        </div>
        {showUserProfile && (
          <div className="text-right" role="complementary" aria-label="Informações do usuário">
            <p className="text-xs text-gray-600">Bem vindo de volta</p>
            <p className="text-sm font-bold text-gray-900" aria-label={`Usuário logado: ${userName}`}>{userName}</p>
          </div>
        )}
      </div>
    </header>
